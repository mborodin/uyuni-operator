...
> **`CLAUDE.md` mirrors this file** for Claude Code. The two are intended to
> stay in sync — if you change durable guidance in one (design principles,
> how-to recipes, pitfalls, conventions), mirror it in the other.
...
internal/uyuni/              # Uyuni JSON API client
  ...
  uyunitest/                 # State-based FakeAPI for reconciler tests (pending)
...
- **API client (`*Client`)**: implemented in `internal/uyuni/client.go`,
  ...
  `/rhn/manager/api/`, `pxt-session-cookie` session, transparent re-auth on
  401).
...
- `RECONCILER_PATCHES.md` has the cleanup intent behind reconcilers that
...
5. Add interface methods to `internal/uyuni/api.go`. Implement on `*Client`
   in `internal/uyuni/client.go` (and on `uyunitest.FakeAPI` once it exists).
6. **Register in `cmd/main.go`** — and make sure the CRD is included in
   ...
   sync (see the implementation-status note above).
...
2. **`internal/controller/`** — reconciler logic against the state-based
   ...
   Tests one Reconcile pass at a time; assert resulting CR + Uyuni state.
...
`cmd/main.go` and the kustomize tree (`config/default`, `config/manager`,
`config/crd`, ...) are in place, so `make run` and `make deploy` work
end-to-end.
...
*Important*: Uyuni XML-RPC API contain namespaces, which documented with a . as separator, i.e. namespace.api.call, but Uyuni instead of . uses /, so the call would be to namespace/api/call
